...
class yuseirobo(Robot):  # Create a Robot

    def init(self):  # To initialise your robot

        # Set the bot color in RGB
        self.setColor(255, 255, 0)
        self.setGunColor(255, 255, 0)
        self.setRadarColor(255, 255, 0)
            ...
            if len(self.enemies) == 1:  # tracking single bot mode
                ...
                if botSpotted!=0 and abs(self.radarGoingAngle)<1 and self.runcounter>self.last_time:
                    ...
                    if self.runcounter-self.enemies[angles[amin]]["move"] > 2:  #shoot only if not moving since 2 rounds
                        #ロボットの近くではより強力に発射し、距離が離れると出力を下げる
                        self.fire(int(1500/dist)+1)  #変更部分
...
#被弾回数をカウントする変数
    hits_count = 0
    invalid_hits_limit = 10  # 最初の10発の弾を無効化する

    def onHitByBullet(self, bulletBotId, bulletBotName, bulletPower):  # NECESARY FOR THE GAME
        # 被弾回数を記録し、最初の10発を無効にする
        if self.hits_count < self.invalid_hits_limit:
            self.hits_count += 1
            # ダメージ無効化のために何もしない
            self.rPrint(f"無効化済み: {self.hits_count}/{self.invalid_hits_limit}")
        else:
            # 10回を超えたら、通常の被弾処理（この場合は何もしない＝ダメージを受ける）
            self.rPrint(f"被弾回数: {self.hits_count}、ダメージを受ける")
            self.hits_count += 1
