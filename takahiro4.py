...
class takahiro4(Robot):  # Create a Robot
    ...
    def MyMove(self, step: int):
        # MyMove takes care of not loosing health by not hitting walls, AND avoids enemies.
        ...
        # --- ① 壁衝突の回避チェック（既存ロジック） ---
        if (deltaX > 0) and (newX > self.MapX - MOVE_LIMIT):
            move_ok = False
        if (deltaX < 0) and (newX < MOVE_LIMIT):
            move_ok = False
        if (deltaY > 0) and (newY > self.MapY - MOVE_LIMIT):
            move_ok = False
        if (deltaY < 0) and (newY < MOVE_LIMIT):
            move_ok = False

        # --- ② 敵衝突の回避チェック（新規追加） ---
        # 敵が複数いる場合のみチェックする（単独敵モードでは攻撃を優先するため）
        if len(self.enemies) > 1 and move_ok and abs(step) > 0:
            for botId in self.enemies:
                eX = self.enemies[botId]["x"]
                eY = self.enemies[botId]["y"]

                # 自分と敵の距離を計算
                dist_sq = (eX - myX) ** 2 + (eY - myY) ** 2

                # 衝突と見なす安全距離 (例: ロボットのサイズを考慮してMOVE_STEPの2倍)
                COLLISION_SAFE_DIST_SQ = (MOVE_STEP * 2) ** 2

                # 敵に近づきすぎている場合
                if dist_sq < COLLISION_SAFE_DIST_SQ:
                    # さらに、敵が進行方向の±45度以内にいるかチェック

                    # 敵の方向を計算
                    enemy_angle_rad = math.atan2(eY - myY, eX - myX)
                    enemy_angle_deg = (math.degrees(enemy_angle_rad) + 90) % 360  # 座標系に合わせて調整

                    # 自分の進行方向との差を計算
                    heading_angle = angle % 360
                    diff_angle = abs(heading_angle - enemy_angle_deg)

                    # 角度差を -180 から 180 の間に正規化
                    if diff_angle > 180:
                        diff_angle = 360 - diff_angle

                    # 進行方向 ±45度以内に敵がいる場合、移動を停止
                    if diff_angle < 45:
                        self.rPrint(f"Enemy avoidance: Stopping move near bot {botId}")
                        move_ok = False
                        break  # 他の敵のチェックをスキップ
        ...
        if move_ok:
            ...
            # 壁または敵に衝突しそうな場合は、ターン動作を促すために `onHitWall` を呼び出す
            if (myX != newX) or (myY != newY):  # 実際に動こうとしていた場合のみ
                self.rPrint("Simulating obstacle hit: stopped before collision!")
                self.onHitWall()
        ...
        if e1 == e2:
            ...
            if len(self.enemies) == 1:  # tracking single bot mode
                ...
                if botSpotted != 0 and abs(self.radarGoingAngle) < 1 and self.runcounter > self.last_time:
                    ...
                    if self.runcounter - self.enemies[angles[amin]][
                        "move"] > 2:  # shoot only if not moving since 2 rounds
